...
LorawanError_t LoRa_Send_Data(void)
{
  ...
  LoRa_ConfigureRadioTx_XYfe(loRa_X.LoRa_sendChannelParameters.dataRate, loRa_X.LoRa_sendChannelParameters.frequency);

  if(RADIO_Transmit_XYfe(loRa_X.LoRa_HeaderBufor, loRa_X.LoRa_HeaderLength) == OK)
    {
      loRa_X.LoRa_StatusDanych = LoRa_transmiting;
      ...
      loRa_X.LoRa_transmitStatus = LoRa_SendData_TX; // set the state of MAC to transmission occurring. No other packets can be sent afterwards
      SwTimerSetTimeout_Yf(loRa_X.LoRa_TimerWaitAck, MS_TO_TICKS_SHORT(LoRa_ACK_timeout));
    }
  else
    {
...
LorawanError_t LoRa_Send_Header_XYfl(void)
{
  ...
  loRa_X.LoRa_Counnter.value++; // the uplink frame counter increments for every new transmission (it does not increment for a retransmission)

  LoRa_ConfigureRadioTx_XYfe(loRa_X.LoRa_ch0_params.dataRate, loRa_X.LoRa_ch0_params.frequency);
  ...
  if(RADIO_Transmit_XYfe(loRa_X.LoRa_HeaderBufor, loRa_X.LoRa_HeaderLength) == OK)
    {
      ...
      //      loRa.lorawanMacStatus.synchronization = ENABLED; //set the synchronization flag because one packet was sent (this is a guard for the the RxAppData of the user)
      loRa_X.LoRa_transmitStatus = LoRa_Handshaking_TX; // set the state of MAC to transmission occurring. No other packets can be sent afterwards
      SwTimerSetTimeout_Yf(loRa_X.LoRa_TimerHandshaking, MS_TO_TICKS_SHORT(LoRa_Handshaking_timeout));
    }
  else
    {
      return LoRa_Send_problem;
...
LorawanError_t LoRa_Send_XYfl(void *buffer, uint8_t bufferLength)
{
  ...
  loRa_X.LoRa_StatusDanych = LoRa_transmit_Fail;
...
void LoRa_EnterReceive_XYfe_HD(void)
{
  ...
  switch(loRa_X.LoRa_transmitStatus)
    {
      case LoRa_Handshaking_TX:
        {
          //          LoRa_EnterReceive_head
          loRa_X.LoRa_transmitStatus = LoRa_Handshaking_RX;
          break;
        }
      ...
      case LoRa_SendData_TX:
        {
          ...
          loRa_X.LoRa_transmitStatus = LoRa_SendData_RX;
          break;
        }
      default:
        loRa_X.LoRa_transmitStatus = ToDo_LoRa_retransmit_radio_configured;
        return;
    }

  LoRa_ConfigureRadioRx_XYfe(loRa_X.LoRa_receiveChannelParameters.dataRate, loRa_X.LoRa_receiveChannelParameters.frequency);

  if(RADIO_ReceiveStart_XYfe(4) != OK)
    {
      loRa_X.LoRa_transmitStatus = ToDo_LoRa_retransmit;
    }
}
  ...
  switch(buffer[1]) //loRa.LoRa_Command
    {
      case 0:
        {
          loRa_X.LoRa_Command = buffer[1];
          LoRa_ConfigureRadioTx_XYfe(loRa_X.LoRa_sendChannelParameters.dataRate, loRa_X.LoRa_sendChannelParameters.frequency);

          if(RADIO_Transmit_XYfe(loRa_X.LoRa_Bufor, loRa_X.LoRa_BuforLength) == OK)
            {
              return Response_OK;
            }
          else
...
LoRaResponseError_t LoRa_DataAckVerify_XYfe(uint8_t *buffer, uint8_t bufferLength)
{
  ...
  //      return Response_CrcError;
  //    }
  if(loRa_X.LoRa_nextUsedChannel != buffer[1])
    {
      ...
      case LoRa_Handshaking_RX:
        {
          if(LoRa_HandshakingAckVerify_XYfe(buffer, bufferLength) == Response_OK)
            {
              loRa_X.LoRa_transmitStatus = LoRa_SendData_TX;
              SwTimerSetTimeout_Yf(loRa_X.LoRa_TimerWaitAck, MS_TO_TICKS_SHORT(LoRa_Transmit_timeout));
            }
          else
            {
              loRa_X.LoRa_transmitStatus = ToDo_LoRa_retransmit_radio_configured;
      ...
      case LoRa_SendData_RX:
        {
          if(LoRa_DataAckVerify_XYfe(buffer, bufferLength) == Response_OK)
            {
              loRa_X.LoRa_transmitStatus = LoRa_Sent;
              SwTimerStop_Yf(loRa_X.LoRa_TimerWaitAck);
              RADIO_SwTimers_stop_Yf();
              //              if(loRa.LoRa_nextUsedChannel == 0)
              {
                loRa_X.LoRa_StatusDanych = LoRa_transmit_OK;
              }
            }
          else
            {
...
static void LoRa_AssemblePacket_XYf(uint8_t *buffer, uint16_t bufferLength, uint8_t nxt_channel)
{
  ...
  memset(&loRa_X.LoRa_Bufor, 0, sizeof(loRa_X.LoRa_Bufor)); //clear bufor
  memset(&loRa_X.LoRa_HeaderBufor, 0, sizeof(loRa_X.LoRa_HeaderBufor)); //clear header

  loRa_X.LoRa_HeaderBufor[bufferHeadIndex++] = loRa_X.LoRa_Addres;
  loRa_X.LoRa_HeaderBufor[bufferHeadIndex++] = nxt_channel;
  //  loRa.LoRa_HeaderBufor[bufferHeadIndex] = LoRa_CRC(loRa.LoRa_HeaderBufor, bufferHeadIndex);
  loRa_X.LoRa_HeaderLength = bufferHeadIndex;

  loRa_X.LoRa_Bufor[1] = 0; //Random(LoRa_Chann_nr) + 1; //nxt channel
  memcpy(&loRa_X.LoRa_Bufor[2], buffer, bufferLength);
  bufferIndex = bufferLength + 3;
  loRa_X.LoRa_Bufor[0] = bufferIndex;
  loRa_X.LoRa_Bufor[bufferIndex] = LoRa_CRC(loRa_X.LoRa_Bufor, bufferIndex);
  loRa_X.LoRa_BuforLength = bufferIndex + 1;
}
...
LorawanError_t LoRa_SetCurrentDataRate(uint8_t valueNew)
{
  ...
  if((valueNew < loRa_X.LoRa_minDataRate) || (valueNew > loRa_X.LoRa_maxDataRate) || (LoRa_ValidateDataRate(valueNew) != OK))
    {
...
{
  return loRa_X.LoRa_currentDataRate;
}
...
{
  return loRa_X.LoRa_txPower;
}

uint8_t LoRa_GetSyncWord(void)
{
  return loRa_X.LoRa_syncWord;
}

void LoRa_SetSyncWord(uint8_t syncWord)
{
  loRa_X.LoRa_syncWord = syncWord;
}

void LoRa_SetCounter(uint32_t ctr)
{
  loRa_X.LoRa_Counnter.value = ctr;
}

uint32_t LoRa_GetCounter(void)
{
  return loRa_X.LoRa_Counnter.value;
}
...
{
  loRa_X.LoRa_batteryLevel = batteryLevelNew;
}
...
{
  loRa_X.LoRa_transmitStatus = LoRa_transmit_Error;
...
void LoRa_PrepareRetransmit_XYfl(void)
{
  ...
  if(loRa_X.LoRa_Counnter.value < LoRa_Retransmit_trials)
    {
      SwTimerSetTimeout_Yf(loRa_X.LoRa_TimerRetransmit, MS_TO_TICKS_LONG(LoRa_Retransmit_timeout));
      SwTimerStart_Yf(loRa_X.LoRa_TimerRetransmit);
      loRa_X.LoRa_transmitStatus = LoRa_Wait_retransmit;
      loRa_X.LoRa_StatusDanych = LoRa_transmiting;
    }
  else
    {
...
{
  loRa_X.LoRa_currentDataRate = valueNew;
}

void LoRa_UpdateTxPower(uint8_t txPowerNew)
{
  loRa_X.LoRa_txPower = txPowerNew;
}
...
{
  uint8_t result;
  result = LoRa_maxPayloadSize[loRa_X.LoRa_currentDataRate];
...
static bool LoRa_FindSmallestDataRate(void)
{
  ...
  if(loRa_X.LoRa_currentDataRate > loRa_X.LoRa_minDataRate)
    {
      dataRate = loRa_X.LoRa_currentDataRate - 1;

      while((found == false) && (dataRate >= loRa_X.LoRa_minDataRate))
        {
          for(i = 0; i < loRa_X.LoRa_maxChannels; i++)
            {
          ...
          if((found == false) && (dataRate > loRa_X.LoRa_minDataRate)) // if no channels were found after one search, then the device will switch to the next lower data rate possible
            {
              dataRate = dataRate - 1;
            }
        }
      ...
      if(found == true)
        {
          loRa_X.LoRa_currentDataRate = dataRate;
        }
    }
